...
typedef struct s_MACHBSTART // 二级引导器收集的信息，会被存在内存1Mb的地方
{
    u64_t mb_migc;         // LMOSMBSP//0
    u64_t mb_chksum;       // 8
    u64_t mb_krlinitstack; // 16 内核栈起始地址
    u64_t mb_krlitstacksz; // 24 内核栈大小
    u64_t mb_imgpadr;      // 操作系统image位置
    u64_t mb_imgsz;        // 操作系统大小
    u64_t mb_krlimgpadr;
    u64_t mb_krlsz;
    u64_t mb_krlvec;
    u64_t mb_krlrunmode;
    ...
    u64_t mb_kservadre;
    u64_t mb_nextwtpadr;
    u64_t mb_bfontpadr;  // 操作系统字体位置
    u64_t mb_bfontsz;    // 操作系统字体大小
    u64_t mb_fvrmphyadr; // 机器显存地址
    u64_t mb_fvrmsz;     // 机器显存大小
    u64_t mb_cpumode;    // 机器cpu模式
